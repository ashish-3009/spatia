...
/**
 * Generate layout slots for `newCount` additional photos, ensuring they don't
 * overlap with the existing slots. Returns a full array: existing + new.
 */
export function extendLayout(
  ...
  if (newCount <= 0) return existingSlots;

  const totalCount = existingSlots.length + newCount;
  const spread = options.spread ?? Math.max(2.5, Math.cbrt(totalCount) * 1.4);
  const depthRatio = options.depthRatio ?? 0.6;
  const scaleMin = options.scaleMin ?? 0.5;
  const scaleMax = options.scaleMax ?? 2.0;
  const minXyDistance = options.minXyDistance ?? 1.2;
  ...
  const minXyDistSq = minXyDistance * minXyDistance;
  // All slots — existing ones are included in collision checks but not regenerated.
  const allSlots: PhotoSlot[] = [...existingSlots];
  ...
  for (let i = 0; i < newCount; i++) {
    ...
    for (let attempt = 0; attempt < maxAttempts && !placed; attempt++) {
      ...
      for (let j = 0; j < allSlots.length; j++) {
        const dx = allSlots[j].position.x - x;
        const dy = allSlots[j].position.y - y;
        if (dx * dx + dy * dy < minXyDistSq) {
          conflicts = true;
          break;
        }
