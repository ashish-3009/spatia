...
import { computeLayout, extendLayout } from '../lib/computeLayout';
  ...
  useEffect(() => {
    ...
    if (photos.length > 0) {
      if (slots && slots.length === photos.length) {
        // Layout already matches — use it as-is (loaded from cloud or already computed).
      } else if (slots && slots.length < photos.length) {
        // Photos were added — extend the layout without reshuffling existing positions.
        slots = extendLayout(slots, photos.length - slots.length);
      } else {
        // No layout or mismatched — compute fresh.
        slots = computeLayout(photos.length);
      }
    ...
    const onTouchMove = (e: TouchEvent) => {
      if (e.touches.length === 2 && initialTouchDist > 0) {
        ...
        targetDistance = Math.max(MIN_DISTANCE, Math.min(MAX_DISTANCE, targetDistance * ratio));
