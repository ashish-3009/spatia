...
interface SpaceState {
  ...
  setCurrentSpace: (space: SavedSpace | null) => void;
}
...
export const useSpaceStore = create<SpaceState>((set, get) => ({
  ...
  saveCurrent: async (name, layoutSeed, photoMeta, files) => {
    ...
    if (currentSpace) {
      ...
      // Figure out which photos are new (have a non-null File) and need uploading.
      const newIndices: number[] = [];
      for (let i = 0; i < files.length; i++) {
        if (files[i] !== null) newIndices.push(i);
      }

      // Upload only the new photos.
      const totalUploads = newIndices.length;
      if (totalUploads > 0) {
        set({ saveProgress: { current: 0, total: totalUploads } });
        for (let uploadIdx = 0; uploadIdx < newIndices.length; uploadIdx++) {
          const i = newIndices[uploadIdx];
          const file = files[i]!;
          try {
            const blob = await makeUploadBlob(file);
          ...
          set({ saveProgress: { current: uploadIdx + 1, total: totalUploads } });
        }
      }

      // Update the database row with new photo_meta.
      ...
      set({ saveProgress: null });
      if (updateError) return { error: updateError.message, id: null };

      // Update currentSpace in the store so subsequent saves still work.
      set({
        currentSpace: {
          ...currentSpace,
          name,
    ...
    // Upload each photo. On any failure, roll back: delete row + storage folder.
    set({ saveProgress: { current: 0, total: files.length } });
    for (let i = 0; i < files.length; i++) {
      const file = files[i];
      if (!file) continue; // Should not happen for new spaces, but safety check.
      try {
        const blob = await makeUploadBlob(file);
        const { error } = await uploadPhoto(user.id, spaceId, photoMeta[i].contentHash, blob);
        if (error) {
    ...
    // Set currentSpace so subsequent saves update instead of creating duplicates.
    set({
      currentSpace: {
        ...(spaceData as SavedSpace),
        photo_meta: photoMeta,
      },
    });
    return { error: null, id: spaceId };
  ...
  loadSpace: async (space) => {
    ...
    if (!user) return { error: 'Not signed in', photos: null, layout: null, files: null, hashes: null };
    ...
    set({ loadProgress: null, currentSpace: space });

    // files: array of null (already uploaded — no local File objects)
    const files: (File | null)[] = photos.map(() => null);
    return { error: null, photos, layout, files, hashes };
  },
  ...
  deleteSpace: async (id) => {
    ...
    // Clear currentSpace if this is the one being deleted.
    if (get().currentSpace?.id === id) set({ currentSpace: null });
    return { error: error?.message ?? null };
  },
}));
