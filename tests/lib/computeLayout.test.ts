...
import { computeLayout, extendLayout } from '../../src/lib/computeLayout';
...
describe('extendLayout', () => {
  it('returns existing slots as-is if newCount is 0 or negative', () => {
    const existing = computeLayout(5);
    expect(extendLayout(existing, 0)).toBe(existing);
    expect(extendLayout(existing, -3)).toBe(existing);
  });

  it('returns N + M slots when extending N slots by M', () => {
    const existing = computeLayout(5);
    const result = extendLayout(existing, 3);
    expect(result).toHaveLength(8);
    // Checks that the first 5 slots are untouched
    expect(result.slice(0, 5)).toEqual(existing);
  });

  it('respects minXyDistance against both existing and new slots', () => {
    const min = 1.2;
    const existing = computeLayout(5, { spread: 3, minXyDistance: min });
    const result = extendLayout(existing, 5, { spread: 3, minXyDistance: min, maxPlacementAttempts: 200 });
  ...
  it('is deterministic when extending with a seeded rng', () => {
    const makeSeededRng = () => {
      let s = 0;
      return () => {
        s = (s * 9301 + 49297) % 233280;
    ...
    const existing = computeLayout(5, {}, makeSeededRng());
    const a = extendLayout(existing, 5, {}, makeSeededRng());
    const b = extendLayout(existing, 5, {}, makeSeededRng());
    expect(a).toEqual(b);
  });
});
